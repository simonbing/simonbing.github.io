---
# layout: archive
title: "Publications"
permalink: /publications/
author_profile: true
---
You can also find my publications on my [Google Scholar page](https://scholar.google.com/citations?user=ydKQzzQAAAAJ&hl).

Conference Papers
------
J L Gamella\*, **S Bing\***, J Runge\\
[Sanity Checking Causal Representation Learning on a Simple Real-World System](https://arxiv.org/abs/2502.20099)\\
*42<sup>nd</sup> International Conference on Machine Learning* (ICML 2025; **Oral**)

**S Bing**, U Ninad, J Wahl, J Runge\\
Identifying Linearly-Mixed Causal Representations from Multi-Node Interventions](https://arxiv.org/abs/2311.02695)\\
*3<sup>rd</sup> Conference on Causal Learning and Reasoning* (CLeaR 2024)

**S Bing**, V Fortuin, G Rätsch\\
On Disenanglement in Gaussian Process Variational Autoencoders](https://arxiv.org/abs/2102.05507)\\
*Advances in Approximate Bayesian Inference* (AABI 2022)

Journal Articles
------
**S Bing**, T Hochsprung, J Wahl, U Ninad, J Runge\\
[Invariance & Causal Representation Learning: Prospects and Limitations](https://openreview.net/forum?id=lpOC6s4BcM)\\
*Transactions on Machine Learning Research* (2024)

**S Bing**, A Dittadi, S Bauer, P Schwab\\
[Conditional Generation of Medical Time Series for Extrapolation to Underrepresented Populations](https://arxiv.org/abs/2201.08186)\\
*PLOS Digital Health 1 (7)* (2022)

Preprints
------
**S Bing\***, J Wahl\*, J Runge\\
[Structural Causal Bottleneck Models](https://arxiv.org/abs/2603.08682)\\
*arXiv preprint*

